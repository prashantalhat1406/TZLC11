...
public class tzlcDataSource
{
    ...
    public void addExpenseToDB(Expenses e)    {
        ContentValues value = new ContentValues();
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_CATEGORY,e.getCategory());
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_AMOUNT,e.getAmount());
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE,e.getDate());
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_TYPE,e.getType());
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE_NUMBER,e.getDate_number());

        long rowID =  database.insert(tzlcDBContract.SaiSevaExpenseDB.TABLE_NAME,null, value);
        Log.d(tzlcDataSource.class.getSimpleName(), "row added " + rowID);
    }

    public List<Expenses> getAllExpenses()    {
        List<Expenses> expenses = new ArrayList<>();
        String selectQuery = "SELECT * FROM expenseDB";
        Cursor cursor = database.rawQuery(selectQuery, null);
        Log.d(tzlcDataSource.class.getSimpleName(), "All Records :  " + cursor.getCount());
        try{
            while(cursor.moveToNext())
            {
                Expenses e  = new Expenses(
                        cursor.getString(cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_CATEGORY)),
                        cursor.getInt(cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_AMOUNT)),
                        cursor.getString(cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE)),
                        cursor.getInt(cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_TYPE))
                );
                e.setDate_number(cursor.getLong((cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE_NUMBER))));
                e.setId(cursor.getLong((cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB._ID))));
                Log.d(tzlcDataSource.class.getSimpleName(), "Expense : category,amount,datenumber, "+e.getCategory()+","+e.getAmount()+","+e.getDate_number());
                expenses.add(e);
            }
        }
        finally {
    ...
    public Expenses getExpenses(long id)    {
        ...
        String selection = tzlcDBContract.SaiSevaExpenseDB._ID + " = ?";
        String[] selectionargs = {String.valueOf(id)};

        Cursor cursor = database.query(tzlcDBContract.SaiSevaExpenseDB.TABLE_NAME,null,selection,selectionargs,null,null,null);
        try{
            while(cursor.moveToNext())
            {
                ...
                expense.setDate_number(cursor.getLong((cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE_NUMBER))));
                expense.setId(cursor.getLong((cursor.getColumnIndex(tzlcDBContract.SaiSevaExpenseDB._ID))));
                Log.d(tzlcDataSource.class.getSimpleName(), "" + expense.getCategory()+","+expense.getDate());
            }
        }
        finally {
            if(cursor != null && !cursor.isClosed())
    ...
    public void updateExpenses(Expenses e)    {
        ...
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_TYPE,e.getType());
        value.put(tzlcDBContract.SaiSevaExpenseDB.COLUMN_DATE_NUMBER,e.getDate_number());
        Log.d(tzlcDataSource.class.getSimpleName(), "" + e.getCategory()+","+e.getDate()+","+e.getId()+","+e.getType());

        String selection = tzlcDBContract.SaiSevaExpenseDB._ID + " = ?";
        String[] selectionargs = {String.valueOf(e.getId())};
        int count = database.update(tzlcDBContract.SaiSevaExpenseDB.TABLE_NAME,value,selection,selectionargs);
        Log.d(tzlcDataSource.class.getSimpleName(), "record updated " + count);
    }

    public void deleteExpenses(Expenses e)    {
        String selection = tzlcDBContract.SaiSevaExpenseDB._ID + " = ?";
        String[] selectionargs = {String.valueOf(e.getId())};
        Log.d(tzlcDataSource.class.getSimpleName(), "" + e.getCategory()+","+e.getDate()+","+e.getId());
        int count = database.delete(tzlcDBContract.SaiSevaExpenseDB.TABLE_NAME,selection,selectionargs);
        Log.d(tzlcDataSource.class.getSimpleName(), "record deleted " + count);
    }

    public List<Expenses> getAllExpensesGroupByCategory(int dateRange) {
        List<Expenses> expenses = new ArrayList<>();
        String selectQuery = "SELECT category, sum(amount)  FROM expenseDB where date_number > "+dateRange+" and date_number < "+(dateRange+100) +" GROUP BY category";
        Cursor cursor = database.rawQuery(selectQuery, null);
        Log.d(tzlcDataSource.class.getSimpleName(), "Summary Records :  " + cursor.getCount());
        try{
            while(cursor.moveToNext())
            {
                Expenses e = new Expenses();
                e.setCategory(cursor.getString(0));
                e.setAmount(cursor.getInt(1));
                Log.d(tzlcDataSource.class.getSimpleName(), "Expense :, "+e.getCategory()+","+e.getAmount()+","+e.getDate_number());
                expenses.add(e);
            }
        }
        finally {
